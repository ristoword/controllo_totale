...
function normalizeDevice(d) {
  return {
    id: d.id || crypto.randomUUID(), restaurantId: d.restaurantId || null, name: String(d.name || "").trim() || "Dispositivo",
...
async function create(data){ const device=normalizeDevice({...data,id:data.id||crypto.randomUUID()}); const list=await readAll(); if(device.isDefault){ list.forEach((d)=>{ if(d.department===device.department) d.isDefault=false; }); } list.push(device); await writeAll(list); return device; }
