...
function normalizeJob(j){ return { id:j.id||crypto.randomUUID(), restaurantId:j.restaurantId||null, eventType:j.eventType||"", department:j.department||"", deviceId:j.deviceId||null, documentTitle:j.documentTitle||"", content:j.content||"", sourceModule:j.sourceModule||"", status:STATUSES.includes(j.status)?j.status:"queued", errorMessage:j.errorMessage||null, relatedOrderId:j.relatedOrderId||null, relatedTable:j.relatedTable||null, createdAt:j.createdAt||new Date().toISOString(), updatedAt:j.updatedAt||new Date().toISOString() }; }
...
async function create(data){ const job=normalizeJob({...data,id:data.id||crypto.randomUUID()}); const list=await readAll(); list.push(job); await writeAll(list); return job; }
