...
function normalizeRoute(r){ return { id:r.id||crypto.randomUUID(), restaurantId:r.restaurantId||null, eventType:String(r.eventType||"").trim(), department:String(r.department||"").trim(), deviceId:r.deviceId||null, isActive:r.isActive!==false, createdAt:r.createdAt||new Date().toISOString(), updatedAt:r.updatedAt||new Date().toISOString() }; }
...
async function create(data){ const route=normalizeRoute({...data,id:data.id||crypto.randomUUID()}); const list=await readAll(); list.push(route); await writeAll(list); return route; }
